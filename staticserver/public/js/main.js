(function ($) {
  ...
  $(function() {
    var sldr = $('.sldr'),
      ...
      course = 1,
      margin = - slideWidth;
    $('.popup_img:last').clone().prependTo('.sldr');
    $('.popup_img').eq(1).clone().appendTo('.sldr');
    $('.sldr').css('margin-left',-slideWidth);

    function nxt_bSlide() {
      var interval=window.setInterval(animate,sldrInterval)
    }
